...
export type ProductPanelImage = {
  asset?: SanityImageAssetReference;
  media?: unknown; // Unable to locate the referenced type "productPanel.image.media" in schema
  hotspot?: SanityImageHotspot;
  crop?: SanityImageCrop;
  alt?: string;
  _type: "image";
...
export type HomeSlideshowVertical = {
...
export type HistoryPageReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
export type StoreLocatorReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
export type SauceIndexReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
export type RecipeIndexReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
export type ProductIndexReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
export type RecipeCategoryReference = {
  _ref: string;
  _type: "reference";
  _weak?: boolean;
...
// Query: *[_type == "homePage" && _id == $id][0]{      _id,  _type,  "title": coalesce(string(select(    defined(ogTitle) => ogTitle,    defined(seoTitle) => seoTitle,    _type == "sauce" => name,    _type == "product" => name,    _type == "recipe" => name,    title  )), ""),  "description": coalesce(string(select(    defined(ogDescription) => ogDescription,    defined(seoDescription) => seoDescription,    _type == "sauce" => pt::text(description),    _type == "product" => coalesce(pt::text(description), name),    _type == "recipe" => select(      defined(name) => "Learn how to make " + name + " with DelGrosso sauces.",      ""    ),    description  )), ""),  "image": select(    _type == "recipe" => mainImage.asset->url,    coalesce(      mainImage.asset->url,      image.asset->url    )  ) + "?w=566&h=566&dpr=2&fit=max",  "dominantColor": select(    _type == "recipe" => mainImage.asset->metadata.palette.dominant.background,    coalesce(      mainImage.asset->metadata.palette.dominant.background,      image.asset->metadata.palette.dominant.background    )  ),  "seoImage": select(    _type == "sauce" => null,    _type == "product" => null,    seoImage.asset->url + "?w=1200&h=630&dpr=2&fit=max"  ),  "date": coalesce(date, _createdAt)  }
...
// Source: ../../packages/sanity-config/src/query.ts
// Variable: getAllSauceSlugsForStaticParamsQuery
// Query: *[    _type == "sauce"    && defined(slug.current)  ] | order(name asc).slug.current
export type GetAllSauceSlugsForStaticParamsQueryResult = Array<string>;
...
// Source: ../../packages/sanity-config/src/query.ts
// Variable: getAllRecipeSlugsForStaticParamsQuery
// Query: *[    _type == "recipe"    && defined(slug.current)    && (      !defined($siteCode)      || $siteCode != "LFD"      ||   count(coalesce(lfdSauces, [])) > 0    )  ] | order(name asc).slug.current
export type GetAllRecipeSlugsForStaticParamsQueryResult = Array<string>;

// Source: ../../packages/sanity-config/src/query.ts
// Variable: getRecipesBySauceIdQuery
// Query: *[    _type == "recipe"    && defined(slug.current)    && $sauceId != null    && references($sauceId)    && (      !defined($siteCode)      || $siteCode != "LFD"      ||   count(coalesce(lfdSauces, [])) > 0    )  ] | order(name asc){    _id,    name,    "slug": slug.current,    tags,    meat,    "categories": array::compact(categories[]->{ _id, title, slug }),    "descriptionPlain": "",    "mainImage": {      "id": coalesce(mainImage.asset._ref, ""),      "preview": mainImage.asset->metadata.lqip,      "hotspot": mainImage.hotspot{ x, y },      "crop": mainImage.crop{ top, bottom, left, right }    },    "sauceLines": array::unique((array::compact(dgfSauces[]->line) + array::compact(lfdSauces[]->line)))  }
...
// Query: *[    _type == "recipe"    && defined(slug.current)    && $sauceIds != null    && count($sauceIds) > 0    && references(*[_id in $sauceIds]._id)    && (      !defined($siteCode)      || $siteCode != "LFD"      ||   count(coalesce(lfdSauces, [])) > 0    )  ] | order(name asc){    _id,    name,    "slug": slug.current,    tags,    meat,    "categories": array::compact(categories[]->{ _id, title, slug }),    "descriptionPlain": "",    "mainImage": {      "id": coalesce(mainImage.asset._ref, ""),      "preview": mainImage.asset->metadata.lqip,      "hotspot": mainImage.hotspot{ x, y },      "crop": mainImage.crop{ top, bottom, left, right }    },    "sauceLines": array::unique((array::compact(dgfSauces[]->line) + array::compact(lfdSauces[]->line)))  }
...
// Query: *[    _type == "recipe"    && slug.current in [$slug, $prefixedSlug]  ][0]{    _id,    _type,    name,    "slug": slug.current,    seoTitle,    seoDescription,    "seoImage": seoImage.asset->url,    seoNoIndex,    ogTitle,    ogDescription,    serves,    tags,    meat,    "categories": array::compact(categories[]->{ _id, title, slug }),    "mainImage": {      "id": coalesce(mainImage.asset._ref, ""),      "preview": mainImage.asset->metadata.lqip,      "hotspot": mainImage.hotspot{ x, y },      "crop": mainImage.crop{ top, bottom, left, right },      "alt": mainImage.alt    },    "video":   select(    defined(video.asset.asset._ref) => {      "playbackId": video.asset.asset->playbackId,      "assetId": video.asset.asset->assetId,      "status": video.asset.asset->status,      "thumbTime": video.asset.asset->thumbTime,      "policy": coalesce(video.asset.asset->data.playback_ids[0].policy, "public"),      "posterImage": video.posterImage{          "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt      }    }  ),    ingredients,    directions,    notes,    "dgfSauces": select(      defined($siteCode) && $siteCode == "LFD" => [],      dgfSauces[]->{        _id,        name,        "slug": slug.current,        line,        "mainImage": mainImage{            "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,          "alt": coalesce(alt, "")        }      }    ),    "organicSauce": select(      defined($siteCode) && $siteCode == "LFD" => null,      organicSauce->{        _id,        name,        "slug": slug.current,        line,        "mainImage": mainImage{            "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,          "alt": coalesce(alt, "")        }      }    ),    lfdSauces[]->{      _id,      name,      "slug": slug.current,      line,      "mainImage": mainImage{          "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,        "alt": coalesce(alt, "")      }    }  }
...
// Source: ../../packages/sanity-config/src/query.ts
// Variable: getAllProductSlugsForStaticParamsQuery
// Query: *[    _type == "product"    && defined(slug.current)  ] | order(name asc).slug.current
export type GetAllProductSlugsForStaticParamsQueryResult = Array<string>;
...
// Query: *[_type == "product" && slug.current in [$slug, $prefixedSlug]][0]{    _id,    _type,    name,    "slug": slug.current,    seoTitle,    seoDescription,    seoNoIndex,    ogTitle,    ogDescription,    sku,    category,    "shippingType": shippingCategory,    price,    weight,    "description": description[]{      ...,      _type == "block" => {        ...,          markDefs[]{    ...,      ...customLink{    openInNewTab,    "href": select(      type == "internal" => internal->slug.current,      type == "external" => external,      "#"    ),  }  }      },      _type == "image" => {          "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,        "caption": caption      }    },    "descriptionPlain": coalesce(pt::text(description), ""),    "mainImage": mainImage{        "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,      "alt": coalesce(alt, "")    },    "sauces": array::compact(sauces[]->{      _id,      _type,      name,      authorName,      line,      category,      "slug": slug.current,      "descriptionPlain": coalesce(pt::text(description), ""),      "mainImage": mainImage{          "id": asset._ref,  "preview": asset->metadata.lqip,  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  },  "alt": alt,        "alt": coalesce(alt, "")      }    })  }
export type GetProductBySlugQueryResult = {
  ...
  sku: string;
  category: "case_of_12" | "gift_pack" | "merchandise";
  shippingType: "gift_pack" | "large_crate" | "normal_item";
  price: number | null;
  weight: number;
  description: Array<
    | {
...
// Source: ../web-dgf/src/lib/sanity/queries.ts
// Variable: dgfRetailersQuery
// Query: *[_type == "retailer"] | order(name asc) {    _id,    _type,    name,    productLinesByState[]{      productLine,      note,      states[]{        label,        value      }    }  }
export type DgfRetailersQueryResult = Array<{
  _id: string;
  _type: "retailer";
  name: string;
  productLinesByState: Array<{
...
// Source: ../web-lfd/src/lib/sanity/queries.ts
// Variable: lfdRetailersQuery
// Query: *[_type == "retailer"] | order(name asc) {    _id,    _type,    name,    productLinesByState[]{      productLine,      note,      states[]{        label,        value      }    }  }
export type LfdRetailersQueryResult = Array<{
  _id: string;
  _type: "retailer";
  name: string;
  productLinesByState: Array<{
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    '\n  *[\n    _type == "sauce"\n    && defined(slug.current)\n  ] | order(name asc).slug.current\n': GetAllSauceSlugsForStaticParamsQueryResult;
    ...
    '\n  *[_type == "retailer"] | order(name asc) {\n    _id,\n    _type,\n    name,\n    productLinesByState[]{\n      productLine,\n      note,\n      states[]{\n        label,\n        value\n      }\n    }\n  }\n':
      | DgfRetailersQueryResult
      | LfdRetailersQueryResult;
