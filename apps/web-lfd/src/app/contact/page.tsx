...
import type { LfdContactPageQueryResult } from "@workspace/sanity-config/types";
...
import { lfdContactPageQuery } from "@/lib/sanity/queries";
import { getSEOMetadata } from "@/lib/seo";

export async function generateMetadata(): Promise<Metadata> {
  const contactData = await sanityFetch({
    query: lfdContactPageQuery,
    params: getSiteParams(),
  });

  const data = (contactData?.data ?? null) as LfdContactPageQueryResult | null;
...
export default async function ContactPage() {
  ...
  type ContactPageData = NonNullable<LfdContactPageQueryResult>;
